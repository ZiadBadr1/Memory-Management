...
public class Algoritm_Worst_Fit {
	int numofPartitons;
	int numofProcess;
	ArrayList<Process> process;
	ArrayList<Partition> partition;
	int counter;
	public Algoritm_Worst_Fit(int numofPartitons,int numofProcess,ArrayList<Process> process,ArrayList<Partition> partition)
	{
		...
		for(int i = 0 ; i < numofProcess;i++)
		{
			...
			for(int j = 0 ; j < numofPartitons;j++)
			{
				...
				{
					if(maxmm < partition.get(j).getSize())
					{
						maxmm = partition.get(j).getSize();
						index = j;
						flage = true;
					}
				}
